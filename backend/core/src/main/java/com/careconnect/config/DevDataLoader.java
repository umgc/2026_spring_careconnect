...
public class DevDataLoader implements CommandLineRunner {
  ...
  private void loadMockData() {
    try {
      log.info("Ã°Å¸â€â€ž Loading mock data from db/migration/mock_data.sql...");

      // Read the SQL file from resources
      ClassPathResource resource = new ClassPathResource("db/migration/mock_data.sql");

      if (!resource.exists()) {
        log.error("Ã¢ÂÅ’ Mock data file not found at: db/migration/mock_data.sql");
      ...
      log.info("Ã¢Å“â€¦ Mock data loaded successfully!");
      log.info("Ã°Å¸â€œÂ§ Login credentials:");
      ...
      log.error("Ã¢ÂÅ’ Failed to load mock data: {}", e.getMessage(), e);
  ...
  private void verifyDataLoad() {
    try (Connection conn = dataSource.getConnection();
      ...
      log.info("Ã°Å¸â€œÅ  Data loaded: {} users, {} patients, {} caregivers, {} medications",
