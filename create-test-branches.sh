...
if [ "$CURRENT_BRANCH" = "main" ]; then
  echo -e "${RED}ERROR: You are on main. Switch to your working branch first.${NC}"; exit 1
fi
...
echo -e "${GREEN}✓ Repo confirmed | Branch: $CURRENT_BRANCH | gh user: $GH_USER${NC}"
...
open_pr() {
  ...
  gh pr create --repo "$REPO" --base "$BASE_BRANCH" --head "$BRANCH_NAME" \
    --title "$PR_TITLE" --body "$PR_BODY" --web
...
This PR contains intentionally clean code designed to make every tool pass.
...
- ✅ OWASP Dependency-Check — no CVEs
...
commit_and_push "test/all-failing" "test: all-failing scenario — violations in every tool"
open_pr "test/all-failing" \
  "[CI TEST] All Tools Failing" \
  "## CI/CD Pipeline Test — All Failing

**Expected results:**
- ❌ TruffleHog — fake AWS key detected
- ❌ Checkstyle — missing Javadoc, style violations
- ❌ SpotBugs — null dereference, resource leak
- ❌ PMD — empty catch, unused vars, deep nesting
- ❌ Semgrep — hardcoded credentials, SQL injection
- ❌ flutter analyze — unused vars, unsafe dynamic
- ❌ OWASP — depends on existing deps

**Merge should be BLOCKED. Do not merge — test branch only.**"
...
**Expected results:**
- ✅ TruffleHog — passes
- ❌ Checkstyle — missing Javadoc, bad naming, line too long
- ✅ SpotBugs — passes
- ✅ PMD — passes
- ✅ Semgrep — passes
...
- ✅ TruffleHog — passes
- ✅ Checkstyle — passes
- ❌ SpotBugs — null dereference, resource leak, ignored return
- ✅ PMD — passes
- ✅ Semgrep — passes
- ✅ flutter analyze — passes
- ✅ OWASP — passes
...
- ✅ Checkstyle — passes
- ✅ SpotBugs — passes
- ❌ PMD — empty catch, unused vars, cyclomatic complexity
- ✅ Semgrep — passes
- ✅ flutter analyze — passes
- ✅ OWASP — passes
...
public class SemgrepViolation {
    /** Hardcoded password — Semgrep: hardcoded-credentials. */
...
commit_and_push "test/fail-trufflehog" "test: TruffleHog detection — fake AWS and API key patterns"
open_pr "test/fail-trufflehog" \
  "[CI TEST] TruffleHog Fails Only" \
  "## CI/CD Pipeline Test — TruffleHog Only

**Expected results:**
- ❌ TruffleHog — fake AWS key and API key patterns detected
- ✅ Checkstyle — passes
- ✅ SpotBugs — passes
- ✅ PMD — passes
- ✅ Semgrep — passes
...
> ⚠️ Workflow uses --only-verified. TruffleHog detects but may not hard-fail
> on inactive keys. Remove --only-verified from extra_args to guarantee failure.
...
# Add vulnerable log4j 2.14.1 (CVE-2021-44228 Log4Shell CVSS 10.0) to pom.xml
...
commit_and_push "test/fail-owasp" "test: only OWASP failure — log4j 2.14.1 CVE-2021-44228 CVSS 10.0"

# Restore pom.xml immediately after push
...
open_pr "test/fail-owasp" \
  ...
  "## CI/CD Pipeline Test — OWASP Only
...
- ✅ Semgrep — passes
- ✅ flutter analyze — passes
- ❌ OWASP — log4j 2.14.1 CVE-2021-44228 Log4Shell CVSS 10.0 HARD BLOCK

> ⚠️ This branch contains a real vulnerable dependency. Do not merge.
...
echo "  1. Go to GitHub Actions tab to watch workflow runs"
echo "  2. Check each PR for the report comment"
echo "  3. Download artifact bundles from Actions tab"
echo ""
